...
class LoginViewModelTests: BaseTestCase {
    ...
    private struct TestInput {
        ...
        static let loginEmpty = TestInput(loginButtonTaps: [()])
        static let loginNonEmpty = TestInput(email: "email", password: "pass", loginButtonTaps: [()])
        static let register = TestInput(registerButtonTaps: [()])
    }
    ...
    @discardableResult private func mockViewModel(for input: TestInput, providers: ProviderDependency = .mock()) -> TestOutput {
    ...
    func testFlowOutputForLoginEmpty() {
        let output = mockViewModel(for: TestInput.loginEmpty)
        scheduler.start()
        XCTAssertEqual(output.flow.events, [])
    }

    func testFlowOutputForLoginCorrectPassword() {
        let output = mockViewModel(for: TestInput.loginNonEmpty)
        scheduler.start()
        XCTAssertEqual(output.flow.events, [
            .next(0, .dismiss)
        ])
    ...
    func testFlowOutputForLoginWrongPassword() {
        ...
        let output = mockViewModel(for: TestInput.loginNonEmpty, providers: providers)
        scheduler.start()
        XCTAssertEqual(output.flow.events, [])
    }

    func testFlowOutputForRegister() {
        let output = mockViewModel(for: TestInput.register)
        scheduler.start()
        XCTAssertEqual(output.flow.events, [
    ...
    func testLoginRequestDidFire() {
        ...
        mockViewModel(for: TestInput.loginNonEmpty, providers: providers)
        scheduler.start()
